...
def classify_and_normalize(
    raw_value: str, forced_type: str = "Auto"
...
def detect_drift(
    ...
    if not authorized_target and any(
        m in AUTHORIZED_ONLY_MODULES for m in modules_requested
    ):
...
def write_audit(event: TelemetryEvent) -> Path:
    ...
    path.write_text(
        json.dumps(asdict(event), indent=2, sort_keys=True), encoding="utf-8"
    )
    return path
...
with gr.Blocks(title=APP_NAME) as demo:
    ...
    with gr.Tab("Audit Export"):
        gr.Markdown(
            "Export a CSV index of audit records. Raw indicators are not included."
        )
        export_button = gr.Button("Export Audit Index")
        audit_index_file = gr.File(label="Audit Index CSV")
        export_button.click(
            fn=export_audit_index, inputs=[], outputs=[audit_index_file]
        )
