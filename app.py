...
from osint_core.drift import recommend_correction
...
def choose_correction(drift: dict[str, float]) -> CorrectionVerb:
    ...
    Delegates to ``osint_core.drift.recommend_correction``, the single source of
    truth for correction thresholds and priority
    (policy > structural > behavioral > adversarial > operational > statistical).
    The UI keeps no parallel threshold table of its own.
    """
    return recommend_correction(drift)
