...
- Correction verbs are limited to: ADAPT, CONSTRAIN, REVERT, OBSERVE.
  “””
...
report_path: str
audit_path: str
constraint_ledger_path: str   # ← new: path written by constraint engine
errors: list[str]
...
if not links:
return “*No source links registered for this indicator type.*”
...
- Constraint Ledger: `{result.constraint_ledger_path}`
...
# Priority table shared by drift and constraint verb merge.

_VERB_PRIORITY: dict[str, int] = {
“OBSERVE”: 0,
“ADAPT”: 1,
“CONSTRAIN”: 2,
“REVERT”: 3,
...
# ------------------------------------------------------------------
# Constraint engine pre-flight
# Evaluates formal invariants (authorization, forbidden capability,
# unknown module, audit-payload shape, correction verb) before any
# module executes.  The ledger entry is written unconditionally so
# the policy decision is recorded independently of run outcome.
# ------------------------------------------------------------------
...
_constraint_ctx = ConstraintContext(
    requested_modules=tuple(selected_modules),
    indicator_hash=indicator_hash,
    indicator_type=indicator_type,
    audit_payload=_audit_payload,
    authorized_target=authorized_target,
    passive_only=not authorized_target,
)
_constraint_report = evaluate_constraints(_constraint_ctx)
_constraint_ledger_path = write_constraint_ledger(
    _constraint_report,
    run_id=run_id,
    base_dir=RUNS_DIR,
)
# Surface enforced constraint violations as run errors.
for _cr in _constraint_report.enforced_violations:
    _msg = f"Constraint [{_cr.code.value}] ({_cr.severity.value}): policy violation"
    if _msg not in errors:
        errors.append(_msg)

# Authorization gate (existing module-level check, kept for defence in depth)
...
# Merge: take the more conservative verb across drift and constraint engines.
# The constraint engine has formal policy authority; drift is heuristic.
_constraint_verb: str = _constraint_report.enforced_correction_verb
if _VERB_PRIORITY.get(_constraint_verb, 0) > _VERB_PRIORITY.get(correction, 0):
    correction = _constraint_verb  # type: ignore[assignment]
...
result = EnrichmentResult(
    ...
    constraint_ledger_path=str(_constraint_ledger_path),
    errors=errors,
)
